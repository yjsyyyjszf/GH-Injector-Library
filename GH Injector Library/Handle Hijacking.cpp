...
std::vector<handle_data> FindProcessHandles(DWORD TargetPID, DWORD WantedHandleAccess)
{
	...
	{
		if (OwnerPID != i.UniqueProcessId && i.UniqueProcessId != TargetPID && i.UniqueProcessId != OwnPID)
		{
			if (hOwnerProc) 
			{
		...
		if (DuplicateHandle(hOwnerProc, hOrig, GetCurrentProcess(), &hDup, PROCESS_QUERY_LIMITED_INFORMATION, FALSE, NULL))
		{ 
